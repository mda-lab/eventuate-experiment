...
public class DDDDefaultStrategy extends AbstractGenerationStrategy {
		...
		if (!hasJMergeTags || appendMode) {
			if (charset != null) {
				if (Charset.isSupported(charset)) {
					writer = new DDDAcceleoFileWriter(file, appendMode, charset);
				} else {
					...
					writer = new DDDAcceleoFileWriter(file, appendMode);
				}
			} else {
				writer = new DDDAcceleoFileWriter(file, appendMode);
			}
			...
			if (charset != null) {
				if (Charset.isSupported(charset)) {
					writer = new DDDAcceleoFileWriter(file.getPath(), charset);
				} else {
					final String message = AcceleoEngineMessages.getString(
							"AcceleoGenerationStrategy.UnsupportedCharset", charset); //$NON-NLS-1$
					AcceleoEnginePlugin.log(message, false);
					writer = new DDDAcceleoFileWriter(file.getPath());
				}
			} else {
				writer = new DDDAcceleoFileWriter(file.getPath());
			}
