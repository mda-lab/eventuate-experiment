...
//		}
		...
		if (target.getName().endsWith(".java")) { //$NON-NLS-1$
			...
			URL resourceURL = DDDJMergeUtil.class.getClassLoader().getResource("templates/emf-merge.xml");
			jmergeFile = URI.createURI(resourceURL.toString()).toString();
			JControlModel model = new JControlModel();
			model.initialize(new DDDASTFacadeHelper(), jmergeFile);
			if (model.canMerge()) {
				try {
